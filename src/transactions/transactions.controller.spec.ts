...
function makeService(): jest.Mocked<TransactionsService> {
  return {
    deposit: jest.fn(),
    withdraw: jest.fn(),
...
describe('TransactionsController', () => {
  ...
  it('withdraw() forwards id, personId, dto, and idempotency key', () => {
    ...
    svc.withdraw.mockResolvedValue({} as never);
  ...
  it('statement() forwards id, personId, and query DTO', () => {
    ...
    svc.getStatement.mockResolvedValue({ items: [] } as never);
