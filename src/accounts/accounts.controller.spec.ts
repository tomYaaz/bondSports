...
function makeService(): jest.Mocked<AccountsService> {
  return {
    create: jest.fn(),
...
describe('AccountsController', () => {
  let svc: jest.Mocked<AccountsService>;
  let ctl: AccountsController;

  beforeEach(() => {
    svc = makeService();
    ctl = new AccountsController(svc);
  });
  ...
  it('findOne() passes the parsed id and personId to AccountsService.findOneForOwner', () => {
    svc.findOneForOwner.mockResolvedValue({} as never);
  ...
  it('block() calls setActive(id, false)', () => {
    svc.setActive.mockResolvedValue({} as never);
